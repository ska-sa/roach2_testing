...
if __name__ == '__main__':
    ...
    p.add_option('-r', '--rate', dest='desired_rate', type='float',default=8,
        help='Set the datarate in Gbps. Default: 9')
    p.add_option('-p', '--pkt_size', dest='pkt_size', type='int',default=1024,
        help='Set the packet size in bytes. Will be rounded to nearest 8 bytes. Default: 1024')
    p.add_option('-s', '--skip', dest='skip', action='store_true',
        help='Skip initialisation and reprogramming of FPGAs.')
    p.add_option('-v', '--debug', dest='debug', action='store_true', default=False,
        help='Print out complete details of each 10GbE core, including ARP tables.')
    p.add_option('-c', '--clk_chk', dest='clk_chk', action='store_false', default=True,
        help='Skip the clock check.')
    p.add_option('-a', '--auto_test', dest='auto_test', action='store_true', default=False,
        help='Automatic test; just run the test for a few seconds and then print a summary')
...
#                    print 'This is our IP or is already accounted for. MAC is %i.'%stat[rx_n]['arp'][ip]
    ...
    desired_dn=opts.desired_rate*1000/200./64.
    ...
    act_rate=act_payld_rate*(on_wire_pkt_size)/opts.pkt_size
    ...
    while(cont>=0):
        ...
        for core in range(8):
            ...
            tx_tib[core]=tx_cnt[core]*(opts.pkt_size+80)/1024/1024/1024.
